...
document.addEventListener("DOMContentLoaded", () => {
	...
	if (toggleButton) {
		toggleButton.addEventListener("click", () => {
			...
			if (isFormVisible) {
				...
				toggleButton.classList.replace("bg-ticketly-orange", "bg-gray-700");
	...
	function closeForm() {
		...
		if (toggleButton) {
			toggleButton.classList.replace("bg-gray-700", "bg-ticketly-orange");
			toggleButton.classList.replace(
				"hover:bg-gray-800",
				"hover:bg-opacity-80"
			);
	...
	function createTicketFormHTML(ticket = null) {
		...
		return `
			<form id="ticket-form" class="space-y-4 bg-white p-6 rounded-lg shadow-md mb-6">
				<h2 class="text-2xl font-bold text-ticketly-green mb-4">${
					isEdit ? "Edit Ticket" : "Create New Ticket"
				}</h2>
				<div>
					<label for="form-title" class="block text-sm font-medium text-gray-700">Title <span class="text-red-500">*</span></label>
					<input type="text" id="form-title" name="title" required class="w-full mt-1 border rounded-md shadow-sm px-3 py-2 focus:outline-none focus:ring-ticketly-orange focus:border-ticketly-orange border-gray-300" value="${escapeHTML(
						ticket?.title || ""
					)}">
				</div>
				<div>
					<label for="form-status" class="block text-sm font-medium text-gray-700">Status <span class="text-red-500">*</span></label>
					<select id="form-status" name="status" required class="w-full mt-1 border rounded-md shadow-sm px-3 py-2 bg-white focus:outline-none focus:ring-ticketly-orange focus:border-ticketly-orange border-gray-300">
						<option value="open" ${
				...
				<div>
					...
					<select id="form-priority" name="priority" class="w-full mt-1 border rounded-md shadow-sm px-3 py-2 bg-white focus:outline-none focus:ring-ticketly-orange focus:border-ticketly-orange border-gray-300">
						<option value="low" ${
				...
				<div>
					...
					<textarea id="form-description" name="description" rows="4" class="w-full mt-1 border rounded-md shadow-sm px-3 py-2 focus:outline-none focus:ring-ticketly-orange focus:border-ticketly-orange border-gray-300">${escapeHTML(
						ticket?.description || ""
					)}</textarea>
				</div>
				<div class="flex justify-end space-x-3">
					...
					<button type="submit" class="px-4 py-2 text-white bg-ticketly-orange rounded-md hover:bg-opacity-80 font-medium">${
	...
	function createTicketCardHTML(ticket) {
		...
		return `
			<div class="bg-white rounded-lg shadow-md border-l-4 ${
				...
				<div class="px-6 py-6 grow"><div class="flex justify-between items-start mb-2"><h3 class="text-xl font-bold text-ticketly-green pr-2 break-words">${escapeHTML(
					ticket.title
			...
			ticket.id
		}" class="edit-button p-2 text-gray-500 rounded-full hover:bg-gray-200 hover:text-ticketly-orange transition-colors" aria-label="Edit ticket">${editIcon}</button><button data-ticket-id="${
			ticket.id
